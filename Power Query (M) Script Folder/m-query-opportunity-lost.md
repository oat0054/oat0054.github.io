...
1. Smart Competitor Logic: I used a "Record" technique to intelligently pick the "winning" competitor's data from several different columns in one efficient pass.

2. Handling Multi-Select Data: Instead of leaving reasons as messy text, I split the values and joined them with a master list to ensure clean and standardized reporting.

3. Performance: I filtered rows early and selected only necessary columns to save memory.
